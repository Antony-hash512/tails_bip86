...
Обязательно укажите флаг версии Python например `--python 3.14`, чтобы C-расширения (модули вроде `crcmod` и `cffi`) были скомпилированы строго под рантайм скаченной версии, а не под системный Python хост-машины:
